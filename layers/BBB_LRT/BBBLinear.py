...
class BBBLinear(ModuleWrapper):
    ...
    def forward(self, x, sample=True):
        ...
        self.act_mu = F.linear(x, self.W_mu, self.bias_mu)
        self.act_var = 1e-16 + F.linear(x ** 2, self.W_sigma ** 2, bias_var)
        self.act_std = torch.sqrt(self.act_var)

        if self.training or sample:
            eps = torch.empty(self.act_mu.size()).normal_(0, 1).to(self.device)
            return self.act_mu + self.act_std * eps
        else:
            return self.act_mu
